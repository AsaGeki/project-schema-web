 ...
 * duração e IP de origem. Fica ligado por `ENABLE_ROUTER_MONITORING`.
 */
export function logRouterMiddleware(req: Request, res: Response, next: NextFunction): void {
  ...
  res.on('finish', () => {
    ...
    const line = `${req.method} ${req.originalUrl} ${res.statusCode} - ${elapsedMs.toFixed(1)}ms ip=${getClientIp(req)}`;
